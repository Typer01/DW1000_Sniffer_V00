...
**Typical workflow:**
1. **Validate the rig before trusting it in the field.** Flash [main/main.c](main/main.c) (the receiver) to your sniffer board, and flash the companion [transmitter/main/main.c](transmitter/main/main.c) — which sends a known, standards-compliant minimal IEEE 802.15.4 Data frame on the exact same config — to a second board. This proves the whole chain end-to-end: DW1000 PHY → receiver firmware → [uwb_live.py](uwb_live.py) → Wireshark's 802.15.4 dissector, against a known-good signal, before you ever point it at something unknown. See [Build / Flash / Monitor](#build--flash--monitor) below.
2. **Capture.** Run `idf.py monitor | python3 uwb_live.py` against the receiver's port (see [uwb_live.py](uwb_live.py) for full usage/options) to turn the live serial log into a `.pcap` you can open in Wireshark, plus a clean text log (`uwb_capture.txt`) with the full per-frame diagnostics.
